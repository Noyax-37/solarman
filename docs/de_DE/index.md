...
Vorstellung
===
Mit dem Solarman-Plugin können Sie die Daten Ihres Wechselrichters abrufen, der über die Website https://home.solarmanpv.com/ überwacht wird, wie beispielsweise die Sofar Solar HYD-Modelle mit 3 bis 6 kES.

Automatische Konfigurationsdateien für: Afore_BNTxxxKTL-2mppt, deye_2mppt, deye_4mppt, deye_hybrid, deye_sg04lp3, deye_string, hyd-zss-hp-3k-6k, kstar_hybrid, sofar_g3hyd, sofar_hyd3k-6k-es, sofar_lsw3, sofar_wifikit, sofar_XXTL-G3.yaml, solis_1p8k-5g, solis_3p-4g, solis_hybrid, solis_s6-EH1P6K-L-PLUS, solis_s6-gr1p, zcs_azzurro-ktl-v3.
<br>

(Unvollständige) Liste der derzeit unterstützten Wechselrichter und der zugehörigen Konfigurationsdatei:
<br>
...
| deye_hybrid.yaml | DEYE/Sunsynk/SolArk-Hybrid-Wechselrichter     | wird verwendet, wenn keine Suche angegeben ist |
...
| kstar_hybrid.yaml | ? | |
| sofar_g3hyd.yaml | SOFAR Hybrid-Dreiphasen-Wechselrichter | HYD 6000 oder unter anderem Markennamen (dreiphasig), z. B. ZCS Azzurro 3PH HYD-ZSS |
| sofar_hyd3k-6k-es.yaml     | SOFAR Hybrid-Einphasen-Wechselrichter | Einphasig, läuft mit Sofar Solar HYD xxxx ES (getestet mit dem 6000) oder umbenannten Modellen, z. B. ZCS Azzurro HYD-ZSS |
...
| solis_s6-EH1P6K-L-PLUS.yaml| SOLIS Hybrid-Wechselrichter, einphasig | |
...
* Schlüssel LSW3 (der am häufigsten verwendete):
...
<br>
<br>
Über eine Schaltfläche können die im Netzwerk vorhandenen Wechselrichter gesucht werden; diese Funktion ist derzeit nur mit den LSW3-Sticks verfügbar:
<br>
...
Sonst nichts Besonderes. Ein Bereich, der für eine mögliche Weiterentwicklung nicht zugänglich ist, wobei noch nicht sicher ist, ob es dazu überhaupt kommen wird.
...
Anlegen eines neuen Equipments
===
![Neue Ausrüstung](ajout_ondul.png)

Klicken Sie auf das „+“ unter „Hinzufügen“
...
Geben Sie Ihrem neuen Gerät einen Namen und wählen Sie anschließend die Vorlagendatei aus, anhand derer es konfiguriert werden soll
...
Danach kann die von Ihnen ausgewählte Konfigurationsdatei nicht mehr bearbeitet werden. Falls Sie sich geirrt haben, löschen Sie dieses Gerät und erstellen Sie ein neues.

Wählen Sie das WLAN-Schlüsselmodell aus, das auf Ihrem Wechselrichter installiert ist (siehe oben).

Sie müssen die IP-Adresse Ihres WLAN-Sticks, den Port, über den er kommuniziert, und seine Seriennummer eingeben. Der Port lautet in der Regel 8899 oder 502, je nach Modell des installierten Sticks, aber die Seriennummer müssen Sie in den Einstellungen Ihres Sticks nachschlagen.

Die Konfigurationsseite können Sie in Ihrem Internetbrowser aufrufen, indem Sie die Adresse Ihres Sticks eingeben: http://adresse_ip_de_votre_clé_wifi. Der Standard-Benutzername und das Standard-Passwort lauten „admin“ und „admin“, sofern Sie diese nicht geändert haben.

![WLAN-Schlüssel-Einstellungen](param_cle.png)
...
Die orangefarbene Schaltfläche setzt die verwendeten Register wieder auf die Werte der verwendeten Konfigurationsdatei zurück. Dies ist erforderlich, wenn Sie die YAML-Konfigurationsdatei ändern. Beachten Sie jedoch: Wenn Fehler angezeigt werden, bleibt Ihnen nichts anderes übrig, als von vorne zu beginnen und ein neues Gerät anzulegen.


Widget-Vorlage
===

Hinzufügen einer Vorlage (danke an @Phpvarious), mit der die wichtigsten Daten des Wechselrichters anschaulicher dargestellt werden können. Zunächst habe ich die Vorlage nur für meinen Wechselrichter (Sofar Solar HYD xxx ES) ergänzt, aber ihr könnt sie selbst anpassen (siehe Konfigurationsdatei unten).

Seit Version 1.0.8 können die Daten, die in die Vorlage einfließen, direkt über die Befehle des Geräts zugewiesen werden => die YAML-Datei muss nicht mehr geändert werden
...
In den Befehlen Ihrer Ausrüstung wird also ein Befehl namens „template“ erscheinen. Sie können selbst entscheiden, ob Sie diese Befehle anzeigen lassen möchten oder nicht.

![Vorlage 1](template1.jpg)

Wenn Sie im Bereich „Einstellungen“ unter „Anzeige“ das verwendete Widget aufrufen.
...
Wert: „Normal“
      -  Schlüssel: 3
Wert: „Status der Entladungsprüfung“
      -  Schlüssel: 4
...
widget: Name des Teils des Widgets, der durch diesen Befehl mit Daten versorgt werden soll (siehe Hilfe unten)  => ab Version 1.0.8 nicht mehr erforderlich
uom: Hier wird in Anführungszeichen die Maßeinheit des im Register gespeicherten Werts angegeben, die in Jeedom verwendet wird.
...
8: Datum und Uhrzeit
9: Uhr
10: Uhrzeit im Sofar-Format (die 8 höchstwertigen Bits ergeben nach Umwandlung in Dezimalzahlen die Stunden, die 8 niedrigstwertigen Bits die Minuten)
11: Datum im Sofar-Format (die 8 höchstwertigen Bits ergeben nach Umwandlung in Dezimalzahlen den Monat, die 8 niedrigstwertigen Bits ergeben den Tag)
12: Ausgabe der Wochentage im SOFOR-Binärformat (auf 7 Bit, wobei das niedrigstwertige Bit den Montag darstellt)
13: Ausgabe im Binärformat
Register: Das oder die Register, in denen die Werte gespeichert sind. Immer in Hexadezimalzahlen und in eckigen Klammern (Tabelle) anzugeben. Bei mehreren Registern sind diese durch ein Komma zu trennen und in der Reihenfolge vom höchsten zum niedrigsten Wert aufzulisten.
isstr: optional, wenn „false“. Gibt bei „true“ an, dass der Wert aus Zeichen besteht und unter Verwendung des unten angegebenen Lookup-Schlüssels übersetzt werden soll.
lookup: Liste der Bedeutung des Registers in Abhängigkeit von seinem Wert.
key: Wert aus der Registrierungsdatenbank, der in Text umgewandelt werden soll
value: Text, der anstelle des Registrierungswerts angezeigt wird
  
Achten Sie darauf, die Struktur dieser Datei genau einzuhalten: die Bindestriche in bestimmten Zeilen und das Fehlen derselben in anderen, die Einrückungen, das Vorhandensein oder Fehlen von Anführungszeichen usw.
...
Bildunterschrift:
Schwarz: Befehle, die in der Ausrüstung eingegeben werden müssen.
