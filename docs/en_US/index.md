...
Overview
===
The Solarman plugin lets you retrieve data from your inverter monitored by the website https://home.solarmanpv.com/, such as the Sofar Solar HYD models ranging from 3 to 6K ES.

Auto-configuration files for: Afore_BNTxxxKTL-2mppt, deye_2mppt, deye_4mppt, deye_hybrid, deye_sg04lp3, deye_string, hyd-zss-hp-3k-6k, kstar_hybrid, sofar_g3hyd, sofar_hyd3k-6k-es, sofar_lsw3, sofar_wifikit, sofar_XXTL-G3.yaml, solis_1p8k-5g, solis_3p-4g, solis_hybrid, solis_s6-EH1P6K-L-PLUS, solis_s6-gr1p, zcs_azzurro-ktl-v3.
<br>

A (non-exhaustive) list of inverters currently supported and their associated configuration files:
<br>
The inverter documentation can be found in [this directory](docs_onduleurs/)
<br>
...
| sofar_hyd3k-6k-es.yaml     | SOFAR Hybrid Single-Phase Inverter | Single-phase, works with Sofar Solar HYD xxxx ES (tested on the 6000) or rebranded models, e.g., ZCS Azzurro HYD-ZSS |
...
To check the log level, it must be set to at least "info": click 1; if the log 3 button does not appear, click 2; and finally, click the 3 button.

Here's the kind of log you'll see appear:

![Network Search Log](log_recherche.png)

All you'll need to do is copy the IP address and serial number into your UPS settings.

Nothing else in particular. An area that isn't open to potential changes, though it's not certain that any will happen.
...
Name your new device, then choose the template file you'll use to configure it
...
You must select the Wi-Fi key model that is installed on your UPS (see above)

You'll need to enter the IP address of your Wi-Fi adapter, the port it uses to communicate, and its serial number. The port is usually 8899 or 502, depending on the model of adapter installed, but you'll need to find the serial number in your adapter's settings.

You can view the configuration page in your web browser by entering your dongle's address: http://adresse_ip_de_votre_clé_wifi. The default username and password—if you haven't changed them—are admin and admin.
...
The orange button resets the used registries to match those in the configuration file. This is necessary if you modify the YAML configuration file. Be careful, however: if errors are displayed, the only option is to start over by recreating a new device.
...
Starting with version 1.0.8, the information used by the template can be set directly from the device controls => no need to edit the YAML file anymore

![widgetgif](widget.gif)

If you installed a version that predates the integration of this widget, or if you modify the configuration file, you must click the "Reset Data" button located in the device settings.

You will therefore see a command called "template" appear in your device's controls. You can choose whether or not to display the commands.

![Template 1](template1.jpg)

If you go to the "Settings" section, you'll see the widget being used.
...
If your inverter is not on the list or if you need to modify the existing file
===

If you think your inverter is one of those that can be monitored by Solarman and you know which Modbus registers to query, then by using the [template file](modele_onduleur.yaml), you can create one yourself, making sure to follow these guidelines:
...
group: Not used in the plugin, but should be kept; the text that follows serves no purpose. It must be kept regardless!
items: same as group. Under this directive, you will group the various registers that you want the plugin to query.
...
11: SOFAR-style date (the 8 most significant bits converted to decimal represent the month, and the 8 least significant bits represent the day)
12: binary representation of the SOFOR weekdays (using 7 bits, with Monday as the least significant bit)
13: Binary representation
registers: the register(s) that store the values. Always specify in hexadecimal and enclosed in square brackets (array). If there are multiple registers, separate them with a comma and list them in descending order from highest to lowest.
isstr: optional if false. If true, indicates that the value is a string and should be translated using the lookup key below
lookup: a list of the registry's meaning based on its value.
key: registry value to be converted to text
value: text that will be displayed in place of the registry value
  
Be sure to follow the structure of this file exactly—including the hyphens on some lines but not on others, the indentation, whether or not to use quotation marks, and so on...
...
Thanks to [@StephaneJoubert](https://github.com/StephanJoubert/home_assistant_solarman), who developed a Home Assistant module from which I obtained some information and very well-written Python files.
...
If you get the error "Cannot uninstall 'PyYAML'. It is a distutils-installed project..." when installing dependencies, go to "Settings/System/Configuration," then to the ">_OS/DB" tab, and finally to ">_System Administration," and run the command "sudo -H pip3 install --ignore-installed PyYAML". Then restart the dependency installation, and everything should work as expected.
